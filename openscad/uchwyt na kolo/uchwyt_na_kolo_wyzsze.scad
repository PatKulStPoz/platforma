...
module mocowanie(h, sr1, gr, dl, sr2){
    ...
    cube([dl+(nakladka_dl-dl)/2, gr, h],center = true);
        }
...
union(){
    ...
    translate([(nakladka_dl-mocowanie_dl)/4,nakladka_sz/2+mocowanie_gr/2,32.5-nakladka_h/2]){
        cube([mocowanie_dl+(nakladka_dl-mocowanie_dl)/2, mocowanie_gr,65],center = true);
    }
