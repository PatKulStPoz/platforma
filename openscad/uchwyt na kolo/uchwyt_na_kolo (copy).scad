...
module mocowanie(h, sr1, gr, dl, sr2){
    difference(){
    translate([(nakladka_dl-dl)/4,0,0]){
        cube([dl+(nakladka_dl-dl)/2, gr, h + 10],center = true);
    }    
...
union(){
    ...
    translate([(nakladka_dl-mocowanie_dl)/4,nakladka_sz/2+mocowanie_gr/2,75 / 2-nakladka_h/2]){
        difference() {
        cube([mocowanie_dl+(nakladka_dl-mocowanie_dl)/2, mocowanie_gr,75],center = true);
            translate([-13, 0, 24.5]) rotate([90,0,0]) {
                cylinder(mocowanie_gr+10, d1=30, d2=16, center=true);
                translate([0, 7.5, 0]) cylinder(mocowanie_gr+10, d=34, center=true);
                translate([0, 5, 0]) cylinder(mocowanie_gr+10, d=32, center=true);
                translate([0, 2.5, 0]) cylinder(mocowanie_gr+10, d=30, center=true);
